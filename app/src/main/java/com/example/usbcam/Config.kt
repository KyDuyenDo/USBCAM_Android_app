...
object Config {
    ...
    /**
     * Timeout cho camera signal detection (ms). Nếu không nhận frame trong khoảng thời gian này →
     * Camera bị lỗi
     */
    const val CAMERA_SIGNAL_TIMEOUT_MS = 3000L

    /** Interval kiểm tra signal health (ms). Watchdog coroutine chạy mỗi 2s */
    const val SIGNAL_CHECK_INTERVAL_MS = 2000L

    /** Delay ban đầu trước khi bắt đầu signal detection. Cho camera thời gian khởi động */
    const val SIGNAL_DETECTION_INITIAL_DELAY_MS = 1000L

    /** Countdown duration (seconds) trước khi preview bắt đầu */
    const val COUNTDOWN_SECONDS = 5

    /** Auto-disable camera khi mất signal */
    ...
    /** USB Video Class (UVC) device class code */
